...
public class MyCart extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Item> list = Dao.getAllCartItem();
        int i=1;
       // String message="<table border='1' width='80%' id='cus_table2'>";
		...
		out.println("<li class='bold'><a><form action='Itemlist' class='waves-effect waves-cyan' method='get'><i class='mdi-editor-border-color'></i><input type='submit' value='Food Menu'></form></a></li>");
		out.println("<li class='bold active'><a><form action='Orderlist' class='waves-effect waves-cyan' method='get'><i class='mdi-editor-insert-invitation'></i><input type='submit' value='Cart'></form></a></li>");
		...
		out.println("<div class='divider'></div><div class='row'><div class='col s12 m4 l3'><h4 class='header'>My Cart</h4></div>");
		out.print("<div class='divider'></div><table border='1' width='50%' id='cus_table2'>");
        
        out.print("<tr><th>SL</th><th>Name</th><th>Price</th><th>Delete</th></tr>");
       for (Item p : list) {
            out.print("<tr><td>" + i + "</td><td>" + p.getName() + "</td><td>" + p.getPrice()+"</td><td><a href='Delete?id="+ p.getId()+"'>Delete</a>"+"</td></tr>");
                    i++;
					
        }
        out.println("<div class='divider'></div>");
        out.print("</table></div>");
		out.println("<div class='divider'></div><div class='row'><div class='col s12 m4 l3'><h4 class='header'>Receipt</h4></div></div>");
		out.println("<div class='divider'></div><div class='row'><center><b><font size='6'> Total ="+Dao.total()+"</font></b></center>");
		out.println("<div class='input-field col s12'><form action='Confirm' method='get' class='btn cyan waves-effect waves-light right' name='action'><input type='SUBMIT' value='Confirm Order'></form></div></div>");
		
		out.println("</section></div></div>");   
